# Computer Selling (Basic)
Project is completed by Baviskop with enough CRUD skills like Add, Edit, Delete, ...
## 📚 Table of contents
- [Giới thiệu](#giới-thiệu)
- [Cài đặt](#cài-đặt)
- [Cách sử dụng](#cách-sử-dụng)
- [Liên hệ](#liên-hệ)

## Giới thiệu
This project marks the completion of the basic Spring Boot functionality with Thymeleaf. Basic functionalities such as adding, editing, and deleting content have been finalized.

## Cài đặt
Hướng dẫn cách cài đặt dự án:
```bash
# Clone repo
git clone https://github.com/baviskop/Computer-Selling-Basic.git

# Di chuyển vào thư mục dự án
cd computer_selling

# Cài đặt dependencies
[Link mvn](https://mvnrepository.com/)
```
## Cách sử dụng

### Login Page
![Login Image](https://github.com/user-attachments/assets/d577aede-0f9a-4581-94bd-49fb763548e4)

### Computers Page For Admin
![Computers Page Image](upload/Screenshot%202026-01-24%20231245.png)

### Computers Page For Staff
![Computers Page Image For Staff](upload/Screenshot%202026-01-24%20231453.png)

### Add New Computer With Validation
![Add Validate Page](upload/Screenshot%202026-01-24%20231402.png)

### Edit Computers
![Edit Computer](upload/Screenshot%202026-01-24%20231319.png)

### Delete Computers
![Delete Computer](upload/Screenshot%202026-01-24%20231329.png)

## Liên hệ
(https://www.linkedin.com/in/thanh-long-6497223a7)
